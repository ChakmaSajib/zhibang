...
class QuestionServive {
  ...
  getAllSetofQuestions() {
    ...
    return http.get('http://localhost:8080/admin/questionSet');
  }
  getAllSetofQuestionsWithUniqueCompany() {
    // it will return all the informations
    return http.get('http://localhost:8080/questionBank/questionSet');
  }

  getSpecificQuestionSet(id) {
    return http.get(
      `http://localhost:8080/questionBank/singleQuestionSet/?id=${id}`
    );
  }
...
export default new QuestionServive();
